...
using WDF_BTL.BaoCao;

namespace WDF_BTL
{
    public partial class CTBan : Form
    {
        ...
        private void LoadDataToGridView(string filter="")
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    {
                        command.CommandText = "Select_CTHDBan";
        ...
        private void cbMaSach_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    command.CommandText = "Select_donGiaBan";
                    command.CommandType = CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("@masach", masach);
        ...
        {
            tbSoHD.Text = tbSoLuongMua.Text = tbTenSach.Text = tbDonGia.Text = string.Empty;
            cbMaSach.ResetText();
        }
        ...
        private void dgv_cthoadon_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            tbSoHD.Text = dataView_gridView[index]["sSoHD"].ToString();
            cbMaSach.Text = dataView_gridView[index]["sMaSach"].ToString();
            tbSoLuongMua.Text = dataView_gridView[index]["iSoLuongMua"].ToString();
        }
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    using (SqlCommand command = connection.CreateCommand())
                    {
                        ...
                        command.CommandType = CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("@maHD", tbSoHD.Text);
                        connection.Open();
                        ...
                        if (idhd_sql != null)
                        {
                            if (idsach != null)
                            {
                                string InsertHD = "Insert_CTHDBan";
                                using (SqlConnection sqlConnection = new SqlConnection(connectionString))
                                {
                                    using (SqlCommand sqlCommand = sqlConnection.CreateCommand())
                                    {
                                        ...
                                        sqlCommand.Parameters.Add("@sl", SqlDbType.Int, 10).Value = tbSoLuongMua.Text;
            ...
            try
            {
                using (SqlConnection sqlConnection = new SqlConnection(connectionString))
                {
                    using (SqlCommand sqlCommand = sqlConnection.CreateCommand())
                    {
                        ...
                        sqlCommand.CommandType = CommandType.StoredProcedure;
                        sqlCommand.Parameters.AddWithValue("@maHD", tbSoHD.Text);
                        ...
                        if (idhd_sql != null)
                        {
                            if (idS_sql != null)
                            {
                                using (SqlConnection connection = new SqlConnection(connectionString))
                                {
                                    using (SqlCommand command = connection.CreateCommand())
                                    {
                                        command.CommandText = "Update_CTHDBan";
                                        command.CommandType = CommandType.StoredProcedure;
                                        ...
                                        command.Parameters.Add("@sl", SqlDbType.Int).Value = tbSoLuongMua.Text;
                            ...
                            if (idS_sql != null)
                            {
                                ...
                                if (dialog_delete == DialogResult.Yes)
                                {
                                    using (SqlConnection connection = new SqlConnection(connectionString))
                                    {
                                        using (SqlCommand command = connection.CreateCommand())
                                        {
                                            command.CommandText = "Delete_CTHDBan";
                                            command.CommandType = CommandType.StoredProcedure;
                                            command.Parameters.Clear();
        ...
        {
            string filter = "sSoHD IS NOT NULL and sMaSach IS NOT NULL";
            if (!string.IsNullOrEmpty(tbSoHD.Text))
            {
                filter += string.Format(" AND sSoHD LIKE '%{0}%'", tbSoHD.Text);
            }
